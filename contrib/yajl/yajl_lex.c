...
yajl_lex_alloc(yajl_alloc_funcs * alloc,
			   unsigned int allowComments, unsigned int validateUTF8, unsigned int allowSloppyFormat)
{
			 ...
			 const unsigned char ** outBuf, size_t * outLen)
{
  ...
  lexed:
	...
	if ((tok == yajl_tok_string || tok == yajl_tok_string_with_escapes) && !skipStrFixup)
	{
		assert(*outLen >= 2);
		(*outBuf)++;
		*outLen -= 2;
