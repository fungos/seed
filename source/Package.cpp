...
IResource *PackageResourceLoader(const char *filename, ResourceManager *res)
{
	...
	bool loaded = pkg->Load(filename, res);
...
u32 Package::GetUsedMemory() const
{
	return IResource::GetUsedMemory() + this->stFile.GetSize() + sizeof(this);
}

bool Package::Load(const char *filename, ResourceManager *res)
{
	ASSERT_NULL(res);
	ASSERT_NULL(filename);

	this->pFilename = filename;
	bool ret = pFileSystem->Open(filename, &stFile);
	...
	{
		ret = this->Load(stFile.GetData(), res);
	}

	return ret;
}

bool Package::Load(const void *data, ResourceManager *res)
{
	UNUSED(res);
...
const char *Package::GetObjectName() const
{
	return "Package"; //this->stFile.GetName();
}

int Package::GetObjectType() const
{
	return Seed::ObjectPackage;
}

void Package::LockUnload()
{
