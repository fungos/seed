...
bool FileSystem::Open(const char *fname, File *file)
{
	file->Close();
    bool result = false;
    // Physfs goes here...
    ...
    /*
	bool result = IFileSystem::Open(fname, file);
	if (!result)
	{
		file->pData = this->Read(fname, &file->iSize);
		file->pName = fname;
...
void *FileSystem::Read(const char *fname, u32 *length)
{
    void *ret = NULL;
	...
	if (fp)
	{
		...
		void *buff = Alloc(this->iLastLength);
...
u32 FileSystem::GetLength() const
{
	return iLastLength;
}
