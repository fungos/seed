...
bool RSXRendererDevice::Initialize()
{
...
bool RSXRendererDevice::Shutdown()
{
	return IRendererDevice::Shutdown();
}

void RSXRendererDevice::BackbufferClear(const PIXEL color)
{
	if (color)
	{
		uPixel p;
...
void RSXRendererDevice::Begin() const
{
...
void RSXRendererDevice::End() const
{
//	glPopMatrix();
	pScreen->ApplyFade();
...
void RSXRendererDevice::SetBlendingOperation(eBlendMode mode, PIXEL color) const
{
	switch (mode)
	{
		/* REPLACE */
...
void RSXRendererDevice::TextureRequestAbort(ITexture *texture)
{
	arTexture.Remove(texture);
}

void RSXRendererDevice::TextureRequest(ITexture *texture)
{
	arTexture.Add(texture);
}

void RSXRendererDevice::TextureRequestProcess() const
{
...
void RSXRendererDevice::TextureUnload(ITexture *texture)
{
//	if (texture->iTextureId)
//		glDeleteTextures(1, &texture->iTextureId);
}

void RSXRendererDevice::TextureDataUpdate(ITexture *texture)
{
	if (texture->iTextureId)
	{
/*
...
void RSXRendererDevice::UploadData(void *userData)
{
...
void RSXRendererDevice::DrawRect(f32 x, f32 y, f32 w, f32 h, PIXEL color, bool fill) const
{
/*
	GLfloat vertices[8];
...
void RSXRendererDevice::Enable2D() const
{
...
void RSXRendererDevice::Disable2D() const
{
/*
